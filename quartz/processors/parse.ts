...
export function createMdProcessor(ctx: BuildCtx): QuartzMdProcessor {
  ...
  return (
    unified()
      ...
      .use(remarkParse)
      // Parse YAML frontmatter (--- blocks) into a `yaml` node
      .use(remarkFrontmatter)
      // Extract frontmatter data from yaml node and set file.data.frontmatter
      .use(() => (tree: MDRoot, file: any) => {
        ...
        visit(tree, "yaml", (node: any, index: number | undefined, parent: any) => {
          ...
          try {
            ...
            // ignore parse errors in frontmatter
          }
          // Remove the yaml node so it doesn't render
          if (parent && index !== undefined) {
            parent.children.splice(index, 1)
          }
        })
        // If no YAML frontmatter, extract title from first H1 heading
        if (!hasFrontmatter) {
          visit(tree, "heading", (node: any, index: number | undefined, parent: any) => {
            if (node.depth === 1) {
              const titleText = node.children?.map((c: any) => c.value).join("") ?? ""
              if (titleText) {
                file.data.frontmatter = { title: titleText }
              }
              // Remove the H1 (article-title component will render it)
              if (parent && index !== undefined) {
                parent.children.splice(index, 1)
              }
              return false // stop after first H1
            }
          })
        }
      })
