#!/usr/bin/env python3
"""从 content/AI提效/ 各篇的元数据行整篇生成 content/AI提效索引.md。

与 gen_english.py 同构(权威源是**篇内元数据行**,索引页是它的视图,勿手编),
只有分级语义与场景表不同 —— 英语域分「能不能用英语干活」,AI 域分
「能不能用 AI 干活」。两个脚本刻意不合并:合并后任何一边加字段都要改另一边,
分开各自演化更省事(重复的只有 40 行渲染骨架)。

元数据行格式(与英语域一致,便于共用校验直觉):
    等级 L2 · 每天 20 分钟 · 场景:写代码→三、让 AI 写业务代码,查bug→四、贴报错的姿势

    - 等级   必填, `L<n>` 或 `L<n>-L<m>`(跨级)
    - 每天   可选, 自由文本(方案/工具这类没有每日投入的篇目可省略)
    - 场景   必填, 逗号分隔; 每项为 `场景名` 或 `场景名→本篇 H2 标题`
             带 → 的生成直达该小节的锚点链接,不带的只链到篇

生成两个视图:
    按等级学 —— L1→L5 路径,每级列出该级篇目(跨级篇目在每一级都出现)
    按场景查 —— 场景 -> 涉及的篇+小节,跨篇聚合(要控成本得同时看基础概念和 API 两篇)

用法:
    python3 scripts/gen_ai.py            # 写入索引
    python3 scripts/gen_ai.py --check    # 只检漂移,退出码 1 表示需要重跑
"""
import os
import re
import sys
...
INDEX = os.path.join(CONTENT, "AI提效索引.md")

META_RE = re.compile(r"^等级\s+(L\d(?:-L\d)?)\s*(?:·\s*每天\s*([^·]+?)\s*)?·\s*场景:(.+)$")

# 等级释义(与 AI提效方案.md 的分级路线图一致,改那边要同步这里)
LEVELS = [
...
SCENE_ORDER = [
    "定级", "选模型", "选工具", "写提示词",
    "写代码", "改代码", "查bug", "写测试", "做评审", "写文档", "写方案",
    "调API", "控成本", "做RAG", "做Agent", "接MCP",
    "防注入", "做评测", "定规范",
]
...
def render(notes):
    ...
    # 同一场景横跨多篇时按起始等级升序 —— 低等级的先看(先懂概念再调 API,不是反过来)
...
def main():
    ...
    if "--check" in sys.argv:
        ...
        if cur.rstrip("\n") != text.rstrip("\n"):
            print("✗ AI提效索引与篇目元数据行漂移,请跑 python3 scripts/gen_ai.py", file=sys.stderr)
            sys.exit(1)
        print("✓ AI提效索引与元数据行一致,无漂移")
        return
