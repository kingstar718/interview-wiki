...
唯一权威源(Explorer 侧栏直接反映目录树)，`知识点索引.md` 底部的
...
HOT_INDEX = os.path.join(CONTENT, "高频题目索引.md")
...
def ai_files():
    """AI 提效域(第四个域,同英语域:不进知识点索引的专题清单,故 D/E 不覆盖它)。"""
...
def check_ai_meta():
    """U. AI 篇元数据行格式 + 场景指向的小节存在 + 索引无漂移(复用 gen_ai,同 T)。"""
    notes = gen_ai.parse_notes(strict=False)
    errors = list(gen_ai.parse_notes.errors)
    if errors:
        return errors
    want = gen_ai.render(notes).rstrip("\n")
    got = read(gen_ai.INDEX).rstrip("\n") if os.path.isfile(gen_ai.INDEX) else ""
    if want != got:
        errors.append("AI提效索引.md 与篇目元数据行漂移,请跑 python3 scripts/gen_ai.py")
    return errors
