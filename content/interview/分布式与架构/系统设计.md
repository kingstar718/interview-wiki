...
**Redis + Lua 令牌桶**（分布式令牌桶，支持突发）：

```lua
-- KEYS[1]=限流key  ARGV[1]=令牌桶容量  ARGV[2]=每秒生成令牌数  ARGV[3]=当前时间戳
local key = KEYS[1]
local capacity = tonumber(ARGV[1])
local rate = tonumber(ARGV[2])
local now = tonumber(ARGV[3])

-- 令牌桶状态：上一次取令牌的时间 + 当时剩余的令牌数
local last_time = redis.call('HGET', key, 'last_time') or now
local stored = tonumber(redis.call('HGET', key, 'stored') or capacity)

-- 计算这段时间生成的令牌数
local elapsed = math.max(0, now - last_time)
stored = math.min(capacity, stored + elapsed * rate)

if stored >= 1 then
    stored = stored - 1
    redis.call('HMSET', key, 'last_time', now, 'stored', stored)
...
```java
// 调用方式（Java 侧）
String lua = "..."; // 上方的 Lua 脚本
List<String> keys = Arrays.asList("ratelimit:api:order");
List<String> args = Arrays.asList("100", "50", String.valueOf(System.currentTimeMillis()));
Long result = redisTemplate.execute(new DefaultRedisScript<>(lua, Long.class), keys, args.toArray());
if (result == 0) { throw new RateLimitException("限流了"); }
```
...
**面试追问：**
- 服务网格一定比传统微服务好吗？→ 不一定。小团队/简单服务引入 Istio 会增加运维复杂度（Envoy 额外资源消耗约 5%-15%、Istio 本身需要专人维护）。服务网格在**大规模、多语言、频繁灰度、强安全**的场景下优势明显。
...
- 如何排查 Trace 中没有的错误？→ Trace 没记录通常是采样率导致的漏采（错误请求概率低赶上不采），或 Span 上报失败。应配合 error log（自带 TraceID）兜底——日志和 Trace 相互补充见[生产排障](生产排障.md)
