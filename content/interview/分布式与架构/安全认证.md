...
| 主问题 | 必讲关键点 | 下一层追问 |
|--------|------------|------------|
| [认证/授权](#认证和授权有什么区别) | 身份与权限分离 | 数据权限、越权、最小权限 |
| [Session](#cookie-和-session-的区别) | 服务端状态、Session ID | 集群共享、固定攻击、强制下线 |
| [JWT](#jwt-的结构是什么) | 声明、签名、有效期 | 算法限制、密钥轮换、撤销 |
| [Access/Refresh Token](#access-token-和-refresh-token-如何配合) | 短期访问、长期换新 | Refresh Token 轮换和重放检测 |
| [OAuth 2.0](#授权码模式的基本流程) | 四个角色、委托授权 | state、redirect_uri、客户端类型 |
| [PKCE](#pkce-解决什么问题) | verifier/challenge 绑定 | 为什么不能替代 state |
| [OIDC](#oauth-20-和-oidc-的区别) | ID Token、身份声明 | Access Token 与 ID Token 能否混用 |
| [SSO](#单点登录-sso-如何实现) | 统一认证、票据、本地会话 | 单点登出、跨域 Cookie |
| [XSS/CSRF](#xss-是什么如何防护) | 脚本执行 vs 借用登录态 | HttpOnly、SameSite、CSP 的边界 |
| [密码存储](#密码应该如何存储) | 慢哈希、盐、成本参数 | 密码迁移、撞库和限速 |
| [常见 Web 攻击](#sql-注入如何防护) | SQL 注入、401/403、API 安全 | 输入过滤、最小权限、鉴权失败处理 |
