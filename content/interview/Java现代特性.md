...
### 1. Java 8、17、21、25 应重点关注什么？（难度：Medium）
...
虚拟线程执行某些操作时可能无法从载体线程卸载，导致载体线程被占用。典型风险包括在 `synchronized` 临界区执行长时间阻塞操作。

治理重点：

- 缩短临界区。
- 不在锁内进行慢 IO。
- 使用诊断工具观察 pinned thread。
- 不把所有平台线程时代的线程池模式原样迁移。

---

## 五、Scoped Value 与上下文传递

### 1. Scoped Value 解决什么问题？
...
具体 API 状态应以项目使用的 JDK 版本为准，避免在生产项目中直接依赖仍处于预览阶段的能力。
