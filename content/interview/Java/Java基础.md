...
| 方法 | 用途 | 何时重写 |
|------|------|---------|
| `equals(Object)` | 判断对象内容相等 | 需要自定义相等逻辑（如 HashMap key） |
| `hashCode()` | 返回哈希码 | 重写 equals 时必须重写 |
| `toString()` | 返回字符串表示 | 日志/调试时方便查看对象状态 |
| `clone()` | 创建并返回副本 | 需要支持克隆（浅拷贝，需实现 Cloneable） |
...
| 修饰对象 | 语义 | 常见场景 |
|---------|------|---------|
| final 类 | 不可被继承 | `String`、`Integer`、安全框架的核心类 |
| final 方法 | 不可被子类重写 | 模板方法模式中锁定算法骨架、防止子类破坏不变量 |
...
|------|-----------------|-----------------|
| 发生位置 | 同一个类（或父子类） | 子类重写父类 |
| 参数列表 | 必须不同（类型/个数/顺序） | 必须相同 |
| 返回值 | 可以不同 | 相同或协变（子类型） |
| 访问修饰符 | 可以不同 | 不能更严格 |
| 异常 | 可以不同 | 不能抛出更宽泛的受检异常 |
| 多态时机 | 编译期 | 运行期 |
| 注解 | 无 | `@Override`（推荐） |
