...
| 主问题 | 必讲关键点 | 下一层追问 |
|--------|------------|------------|
| [JMM](#jmmjava-内存模型) | 主内存/工作内存、可见性、有序性 | [happens-before](#happens-before-规则)、内存屏障 |
...
| [CAS](#cas-的原理和问题) | 比较交换、无锁更新 | ABA、自旋开销、LongAdder、AtomicStampedReference |
...
| [正确停止线程](#如何正确停止线程) | 协作式中断、三种方式、守护线程 | interrupt 三方法、吞掉中断的后果 |
| [锁分类](#锁的分类总结) | 乐观/悲观、共享/独占、公平/非公平 | 可重入实现、synchronized 是公平吗 |
| [StampedLock](#stampedlock-是什么与-readwritelock-相比有什么优势) | 乐观读、stamp 验证 | 与 ReadWriteLock 对比、不可重入 |
| [死锁](#什么是死锁如何避免) | 必要条件、避免策略 | 排查工具、预防方法 |
| [活锁](#什么是活锁如何避免) | 互相谦让、无法推进 | 与死锁的区别、随机退避 |
| [锁优化](#jvm-对-synchronized-的优化) | 锁粗化、锁消除、自适应自旋 | JIT 如何自动优化锁 |
...
happens-before 是 JMM 定义的**偏序关系**，用来判断两个操作之间是否存在**内存可见性保证**：如果 A happens-before B，则 A 的结果对 B 可见，且 A 的执行顺序在 B 之前。
...
5. **线程启动规则**：`Thread.start()` hb 线程内的任何操作
6. **线程终止规则**：线程内的所有操作 hb `Thread.join()` 返回
7. **线程中断规则**：`interrupt()` hb 被中断线程检测到中断
8. **对象终结规则**：构造函数结束 hb `finalize()` 开始

**面试怎么用**：happens-before 不是"实际执行顺序"，而是**内存可见性的保证**。比如规则 3 保证了 `synchronized` 块内的修改在释放锁后对下一个获取锁的线程可见，这就是为什么 `synchronized` 能保证可见性——不是因为"互斥"，而是因为 happens-before 建立了可见性链条。
...
Java 用**协作式中断**：一个线程请求另一个线程"请停止"，被请求的线程在合适的时机检查中断标志并自行退出。`Thread.stop()` 已废弃（强制终止导致不一致状态）。

**三种方式**：
- **共享标志位**：用 `volatile boolean` 控制循环退出
- **中断机制**：`Thread.interrupt()` 设置中断标志，阻塞操作（`wait/sleep/join`）会抛 `InterruptedException` 并清除标志
- **Future.cancel()**：线程池提交任务后通过 Future 取消

**中断相关方法**：
- `thread.interrupt()`：设置中断标志 + 唤醒阻塞线程
...
守护线程（Daemon Thread）是"服务线程"，当 JVM 中只剩下守护线程时，JVM 自动退出。通过 `thread.setDaemon(true)` 设置（必须在 `start()` 之前调用）。

**特点**：GC 线程是典型的守护线程；守护线程的 finally 块不保证执行（JVM 退出时直接终止）；守护线程创建的子线程默认也是守护线程。
...
**自适应自旋**：根据上次自旋结果动态调整自旋次数，上次自旋成功则延长，失败则缩短或直接阻塞。
...
**是什么**：Java 8 引入的读写锁的改进版，引入**乐观读**模式。常规读写锁的读操作需要获取读锁（CAS 操作有开销），StampedLock 的乐观读**不加锁直接读**，读完校验 stamp 是否变化——如果期间没有写操作，乐观读零开销完成。
...
**是什么**：活锁（Livelock）与死锁不同——线程没有被阻塞，而是在不断**改变状态尝试避开冲突**，但永远无法推进。典型场景：两个线程互相谦让资源，A 发现 B 在用就释放，B 发现 A 在用也释放，两人无限循环。
...
**是什么**：CPU 缓存以**缓存行**（Cache Line，通常 64 字节）为单位加载。两个线程分别修改同一缓存行上的不同变量时，虽然逻辑上不共享数据，但 CPU 缓存一致性协议（MESI）会强制该缓存行在两个 CPU 核之间来回失效和同步，导致性能严重下降。

**示例**：`volatile long a, b;` 紧挨着放在内存中，线程 1 写 a、线程 2 写 b，两个变量在同一缓存行，每次写入都触发对方 CPU 缓存行失效。

**解决方案**：
- **填充（Padding）**：在变量前后加无意义的 long 字段，把目标变量推到一个缓存行独占的位置。如 Disruptor 的 `RingBuffer` 在 cursor 前后各填充 7 个 long（7×8=56 + 8=64 字节）
- **`@Contended` 注解**（Java 8+）：JVM 自动为注解的字段添加填充，需加 JVM 参数 `-XX:-RestrictContended` 解锁。`ConcurrentHashMap` 的 `CounterCell` 和 `Thread` 的 `threadLocalRandomSeed` 就用了这个注解

**常见追问**：伪共享和共享有什么区别？→ 共享是**有意**访问同一变量，伪共享是**无意**被缓存行牵连——两个线程改的是不同变量，但因缓存行粒度被迫"共享"。
...
AtomicLong 在高并发下所有线程 CAS 同一个 `volatile long`，冲突率高、自旋开销大。`LongAdder`（Java 8+）将单一值拆成**多个 Cell**（每个 Cell 里一个 long），线程按 `Thread.threadLocalRandomProbe` 哈希到不同 Cell，CAS 分散到不同内存位置，竞争大幅降低。

**原理**：空间换时间——和 `ConcurrentHashMap` 分段锁、`ThreadLocal` 线程私有副本思路一致。`sum()` 遍历所有 Cell 求和（非原子快照，并发写时 sum 可能不精确），所以 `LongAdder` 适合**统计计数**场景，不适合需要精确瞬时值的场景（如余额）。

**常见追问**：什么时候用 AtomicLong、什么时候用 LongAdder？→ 低竞争用 AtomicLong（简单、精确），高竞争统计用 LongAdder（吞吐高但 sum 不精确），需要精确瞬时值还是要用 AtomicLong 或锁。
...
- 与 CountDownLatch 的区别？→ Semaphore 是**可复用的计数器**（acquire/release 反复使用），CountDownLatch 是**一次性**的（countDown 到 0 后不可重置）
...
**通用概念**：ThreadLocal 是**用空间隔离换同步消除**——每线程一份副本，不共享就无竞争。同类模式：JVM 的 TLAB（线程私有分配缓冲，见[JVM](JVM.md)）、Netty 线程绑定 Arena（见[Netty与RPC](Netty与RPC.md)内存池一节）、LongAdder 的分段 Cell。但它只是上下文传递的第一层——不跨线程（线程池要捕获/回放，见上方 InheritableThreadLocal/TTL）、更不跨进程（要靠协议头捎带，如链路追踪的 traceId）。Spring 事务上下文、MDC 日志上下文都构建在 ThreadLocal 之上（见[Spring](Spring.md)事务详解）。
