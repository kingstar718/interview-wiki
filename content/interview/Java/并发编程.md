...
**通用概念**：ThreadLocal 只是[上下文传递](上下文传递.md#进程内threadlocal)的第一层——它不跨线程(线程池要捕获/回放)、更不跨进程(要靠协议头捎带)。Spring 事务传播与链路追踪都建立在这套机制上。
...
**通用概念**：拒绝策略是[背压](背压.md#线程池有界队列才有背压)的四种形态,其中 CallerRunsPolicy 最优雅:提交者忙于执行任务,自然就没空提交新任务。禁用 Executors 的真正理由是它的无界队列根本没有背压,而不是「参数不清晰」。
