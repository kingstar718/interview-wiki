...
| 类型 | 特点 | 线程安全 | 场景 |
|------|------|--------|------|
| **ArrayList** | 数组实现，随机访问快 O(1) | ❌ | 频繁查询 |
| **LinkedList** | 链表实现，插删快 O(1) | ❌ | 频繁插删 |
| **HashMap** | 数组 + 链表/红黑树 | ❌ | 高性能 KV |
| **ConcurrentHashMap** | 分段锁/CAS | ✅ | 并发 KV |
| **TreeMap** | 红黑树，有序 | ❌ | 有序遍历 |
| **HashSet** | HashMap 实现 | ❌ | 去重 |
...
**扩展**
- 为什么负载因子是 0.75？时间和空间的平衡
- 为什么要转红黑树？避免链表过长导致性能退化
- HashMap 为什么不是线程安全的？写操作没有同步
...
为了保持哈希桶均匀分布。如果容量从 n 增加到 n+1（不是 2n），则：

```
hash(key) % n  // 原始桶位置
hash(key) % (n+1)  // 新桶位置，分布不均
```

只有容量是 2 的幂时，`(n-1) & hash` 才能均匀分布。

---

#### 为什么链表转红黑树的阈值是 8？

泊松分布计算表明，在负载因子 0.75 下，一个桶内有 8 个元素的概率极低（约千万分之一）。如果出现了，说明哈希冲突严重，应该转树避免 O(n) 链表遍历。
...
**❌ 常见误区：**
1. 使用 Vector 或 synchronized List（过时的同步方案）
2. HashMap 放在多线程环境（use ConcurrentHashMap）
3. 在遍历时修改列表（会抛 ConcurrentModificationException）
4. 初始化时不指定容量（导致频繁扩容）
