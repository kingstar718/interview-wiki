...
**通用概念**：稀疏索引是"**粗定位 + 顺序扫尾**"模式——每层只负责把范围缩小到下层能顺序处理的粒度。跳表每一层就是下一层的稀疏索引（见[Redis](Redis.md)跳表实现原理），ClickHouse 的主键索引同样按 granule（8192 行）稀疏建；对立面是哈希/稠密索引：O(1) 点查但完全放弃顺序局部性。
