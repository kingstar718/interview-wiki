...
| 主问题 | 必讲关键点 | 下一层追问 |
|--------|------------|------------|
| [Znode](#zookeeper-的数据模型是什么) | 持久/临时/顺序 | 会话过期后节点何时删除 |
| [Session](#zookeeper-session-如何工作) | 心跳、超时、重连 | 断连与过期区别 |
| [Watch](#watch-机制有什么特点) | 通知、一次性、重新读取 | 丢事件和羊群效应 |
| [分布式锁](#zookeeper-如何实现公平分布式锁) | 临时顺序节点、监听前驱 | 公平性、会话过期、fencing |
| [ZAB](#zab-的广播流程) | Leader、Proposal、ACK、Commit | Leader 故障、zxid |
| [集群节点数](#zookeeper-为什么推荐奇数节点) | 多数派和容错 | 为什么 4 节点不优于 3 节点 |
| [注册发现](#注册中心的核心职责是什么) | 注册、健康、订阅、本地缓存 | 注册中心不可用如何继续调用 |
| [Nacos/ZK/Eureka](#nacoszookeepereureka-如何比较) | 一致性、生态、功能 | 如何结合项目约束选型 |
| [配置中心](#配置变更如何保证安全) | 版本、推送、灰度、回滚 | 多实例短时不一致 |
| [Leader 选举](#zookeeper-如何实现-leader-选举) | 选主流程、zxid 比较、epoch | 临时节点删除与业务安全退出 |
...
### Nacos、ZooKeeper、Eureka 如何比较？

| 组件 | 一致性模型 | 特点 | 常见场景 |
|------|-----------|------|----------|
| Nacos | **AP/CP 可切换**（`ephemeral=true` 走 Distro 协议 AP，`false` 走 Raft CP） | 注册中心 + 配置中心，支持多种实例模型 | Spring Cloud Alibaba |
| ZooKeeper | **CP**（ZAB 协议，多数派写成功才确认） | 协调语义强，临时节点和 Watch | Dubbo、选主、分布式协调 |
| Eureka | **AP**（拒绝因网络分区牺牲可用性，自我保护模式） | 客户端缓存、去中心化对等复制 | 存量 Spring Cloud Netflix 项目 |
