...
| 主问题 | 必讲关键点 | 下一层追问 |
|--------|------------|------------|
| [生命周期](#maven-的生命周期有哪些) | clean/default/site、阶段顺序 | Phase 与 Goal 的绑定 |
| [Scope](#maven-依赖范围有哪些) | 编译、测试、运行、传递 | provided 在 Spring Boot jar 中的影响 |
| [依赖冲突](#maven-如何解决依赖冲突) | 最近优先、先声明 | dependencyManagement、排除 |
| [BOM](#bom-是什么) | 统一兼容版本 | import scope、与 parent 的区别 |
| [多模块](#maven-聚合和继承有什么区别) | 聚合、继承、依赖方向 | 循环依赖如何治理 |
| [Snapshot](#snapshot-和-release-的区别) | 可变开发版本 | 仓库更新策略、为何生产禁用 |
| [可重复构建](#什么是可重复构建) | 固定版本、统一工具、同一制品 | 时间戳、环境差异 |
| [供应链安全](#如何治理依赖漏洞) | 漏洞扫描、SBOM、仓库治理 | 依赖混淆、传递依赖漏洞 |
| [构建优化](#maven-构建慢怎么优化) | 缓存、增量、并行、测试分层 | 如何保证优化后结果一致 |
