...
| 主问题 | 必讲关键点 | 下一层追问 |
|--------|------------|------------|
| [REST](#什么是-restful-api) | 资源、Method、状态码、无状态 | PUT/PATCH、复杂动作如何表达 |
| [错误码](#如何设计稳定的错误码) | 稳定、分层、traceId | HTTP 码与业务码如何配合 |
| [分页](#分页接口如何设计) | offset vs cursor、稳定排序 | 数据插入导致翻页重复/遗漏 |
| [幂等](#创建接口如何保证幂等) | 稳定 Key、请求摘要、结果复用 | 并发首请求、记录过期 |
| [乐观锁](#乐观锁如何用于接口更新) | version/ETag、条件更新 | 冲突后如何合并 |
| [版本管理](#api-如何做版本管理) | 向后兼容、弃用流程 | 枚举新增、字段删除 |
| [API 签名](#开放-api-如何设计签名) | 规范化请求、时间戳、Nonce | 参数顺序、Body 摘要、密钥轮换 |
| [防重放](#如何防止重放攻击) | 时间窗口、Nonce、幂等 | 多机如何共享 Nonce |
| [限流](#api-限流维度有哪些) | 维度、算法、返回语义 | Retry-After、分布式限流 |
| [超时](#如何设计-api-超时) | 总预算、逐层递减 | 重试如何落入预算 |
| [OpenAPI](#openapi-有什么作用) | 契约、生成、CI 检查 | 文档与实现漂移 |
