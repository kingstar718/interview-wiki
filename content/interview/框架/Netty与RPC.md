...
| 主问题 | 必讲关键点 | 下一层追问 |
|--------|------------|------------|
| [Reactor](#netty-的线程模型是什么) | 事件分发、EventLoop、Channel 绑定 | 单/多 Reactor、为何减少锁 |
| [EventLoopGroup](#eventloopgroup-是怎么管理线程的) | 线程管理、chooser 策略、Channel 绑定 | boss vs worker、runAllTasks |
| [Pipeline](#netty-的线程模型是什么) | 入站/出站 Handler 链 | 执行顺序、异常传播、Sharable |
| [ByteBuf](#bytebuf-相比-bytebuffer-有什么优势) | 读写索引、池化、直接内存 | 引用计数、切片、泄漏检测 |
| [零拷贝](#netty-的零拷贝体现在哪) | FileRegion、CompositeByteBuf、wrappedBuffer、slice | OS 层 vs Netty 层、小文件场景 |
| [内存池](#netty-内存池是怎么分配的pooledbytebufallocator) | Arena/Chunk/Subpage、伙伴算法 | 线程绑定 Arena 为何减少竞争、小对象为何走 Subpage |
| [粘包拆包](#tcp-粘包拆包如何解决) | 字节流、协议边界 | 长度字段偏移、恶意长度 |
| [EventLoop 阻塞](#为什么不能在-eventloop-中执行耗时任务) | 一个线程管理多个连接 | 业务线程池、任务回写 |
| [心跳重连](#如何设计心跳和空闲连接检测) | Idle 检测、退避、状态恢复 | 网络抖动、重连风暴 |
| [RPC 代理](#rpc-框架的核心组成) | 本地调用转远程请求 | 泛化调用、异常映射 |
| [服务发现](#rpc-框架的核心组成) | 实例缓存、订阅变更 | 注册中心不可用时如何调用 |
| [超时重试](#rpc-重试要注意什么) | 总超时、幂等、退避 | 重试风暴、对冲请求 |
| [序列化](#常见序列化方式如何选择) | 性能、兼容、跨语言 | Schema 演进、安全风险 |
...
**是什么**：`EventLoopGroup` 管理一组 `EventLoop`（每个 EventLoop 绑定一个**线程**，线程处理多个 Channel 的 IO 事件）。以 `NioEventLoopGroup` 为例：
...
2. **Channel 绑定一次**：`bossGroup.register(channel)` 将一个 Channel 注册到某个 EventLoop 后，该 Channel 的**所有 IO 事件都由这一个 EventLoop 的线程串行处理**——这是 Netty 减少锁竞争的核心手段（见 [并发编程](并发编程.md) 中线程封闭原则）。

3. **chooser 选择策略**：默认 `DefaultChooser` 用 `++idx` 取模轮询；`PowerOfTwoChooser`（可选）在 EventLoop 数量为 2 的幂时用 `idx & (n-1)` 代替取模。实测性能差异很小，Netty 4.x 默认已统一为 `GenericChooser`。
...
**是什么**：零拷贝的核心是**减少数据在内核空间和用户空间之间的复制次数**，而不是完全不拷贝。
...
| `CompositeByteBuf` | 多个 ByteBuf **逻辑组合**为连续视图，无需逐个拷贝到新数组 | 协议头 + 协议体拼接发包 |
| `Unpooled.wrappedBuffer()` | 将已有 `byte[]` / `ByteBuffer` **包装**成 ByteBuf，底层是同一份数据 | 将业务层 byte[] 转换为 Netty 对象发送 |
...
**通用概念**：零拷贝的核心思维是**消除不必要的内存副本**，同样的思路出现在 Kafka 的 `transferTo` 消费日志、[RocketMQ](消息队列.md) 的 mmap 写 CommitLog、以及 [MySQL](MySQL.md) 的 Doublewrite Buffer 选择直接写而非先缓存再刷——都是在"数据路径上加副本 vs 不加副本的风险"之间做权衡。
