...
**通用概念**：补偿与重试都建立在[幂等](幂等.md)之上——不幂等的接口不能安全重试。
...
**通用概念**：链路追踪本质是[跨进程的上下文传递](上下文传递.md#跨进程协议头捎带)——进程内靠 ThreadLocal，跨线程靠捕获/回放，跨进程靠协议头捎带。与 Spring 事务传播行为（线程内的上下文传递策略，见[Spring](Spring.md)）是同一模式在不同作用域的形态。
