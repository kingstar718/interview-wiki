...
| [HTTP 缓存](#什么是强缓存和协商缓存它们的工作原理是什么) | 强缓存、协商缓存、Cache-Control | ETag vs Last-Modified、no-cache vs no-store |
| [WebSocket](#websocket-是什么它在网络通信中有什么作用) | Upgrade 握手、帧格式、全双工 | 心跳机制、与 HTTP 长连接的区别 |
| [IP/MAC/ARP](#ip-地址和-mac-地址有什么区别在网络通信中各自扮演什么角色) | 逻辑地址 vs 物理地址、ARP 广播 | ARP 欺骗、为什么需要两套地址 |
| [NAT](#nat网络地址转换是什么有什么作用) | 地址转换表、端口映射 | NAT 穿透、STUN/TURN |
| [CDN](#cdn-是什么它在网络传输中有什么作用) | 边缘节点、DNS 调度、回源 | 缓存刷新、预热、与浏览器缓存的关系 |
...
**SACK（Selective Acknowledgment）详解**：快速重传的缺陷是——发送方收到 3 个重复 ACK 后，只知道"有包丢了"，但不知道是**哪一个**丢了（TCP 累积确认只能确认连续收到的最大序号）。如果发送了 1~10 包，只收到 1、2、3、5、6、7 的 ACK，发送方不知道 4 丢了，需要全部重传。

SACK 在 TCP 选项字段中携带已收到的**不连续块**的边界，发送方精确知道哪些包丢了，只重传缺失的段。SACK 在 Linux 默认开启（`net.ipv4.tcp_sack = 1`）。**D-SACK** 是 SACK 的扩展，让发送方区分"包丢了"和"ACK 丢了"。

**SACK 的代价**：SACK 选项占用 TCP 头部空间，每段不连续块需要 8 字节（开始 + 结束序号），头部空间有限，丢包过多时 SACK 无法完整描述所有空洞，会退化为部分重传。
