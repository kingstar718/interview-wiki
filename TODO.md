...
### AI提效域:剩余 8 篇正文(2026-08-05 登记)

骨架、`gen_ai.py`、校验 U、索引、首批四篇已完成(见「已完成」)。**分级定义在 [AI提效方案](content/AI提效/AI提效方案.md),写新篇前先对齐等级语义**;元数据行场景名要在 `gen_ai.py` 的 `SCENE_ORDER` 里登记,否则会排到场景表末尾。

把 LLM 接进系统(L3-L4,后端差异化,也是新面试考点):

- [ ] `LLM应用开发.md` — 流式、超时重试、并发、**token 计费与成本控制**、prompt caching 落地、结构化输出/Function Calling(场景:调API/控成本)
- [ ] `RAG检索增强.md` — 为什么需要、切块/嵌入/向量库选型、召回+重排、怎么评估效果(场景:做RAG)
- [ ] `Agent与MCP.md` — Agent 循环与成本陷阱、工具调用、MCP 是什么、**什么时候不该上 Agent**(场景:做Agent/接MCP)

治理(L5):

- [ ] `AI工程化与风险.md` — 提示注入、数据边界红线、幻觉治理、评测集与可观测、团队规范(场景:防注入/做评测/定规范)

> 写作提示:四篇已完成的正文里有多处指向这 8 篇的**纯文字引用**(未加链接,避免死链)。补完对应篇目后,回头把这些引用改成链接 —— `grep -n 'LLM应用开发\|RAG检索增强\|Agent与MCP\|AI工程化与风险' content/AI提效/*.md` 可一次找全。

发现新缺口先登记到这里再做 —— 写法模板与同步清单见 [CONTRIBUTING.md](./CONTRIBUTING.md)。
