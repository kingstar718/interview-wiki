...
- [x] 阶段0 骨架 — `content/软考系统架构师/` 目录 + `scripts/gen_rk.py` 双视图索引生成器(按科目学/按场景查)+ `check_index.py` 校验 V(元数据行/场景小节/索引漂移)+ C/G/H 接入 + `quartz.ts` 侧栏登记 + 首页/CLAUDE/CONTRIBUTING/README/TODO 同步(commit 短哈希待提交时补)
- [x] 软考备考总览.md — 三科形式(机考/连考/及格线 45)+ 2026 下半年时间线(10/24-25 考试、8/21-9/3 报名)+ 新版大纲变化 + 八周备考路线 + 官方渠道(commit 短哈希待提交时补)
- [x] 重点分布与分值.md — 综合知识 14 知识域分值分布表(架构设计/软件架构设计为绝对核心)+ 案例题型分布 + 论文主题分布,每域标 ★(commit 短哈希待提交时补)
- [x] 综合知识·计算机与软件工程.md — 计算机系统基础/操作系统/网络/数据库/软件工程/项目管理/知识产权(commit 短哈希待提交时补)
- [x] 综合知识·架构核心.md — 架构风格/架构模式/质量属性与评估(ATAM)/4+1 视图/ADL/构件与中间件(最高权重)(commit 短哈希待提交时补)
- [x] 综合知识·信息安全与可靠性.md — 加解密/密钥管理/访问控制/安全体系/可靠性计算(commit 短哈希待提交时补)
- [x] 综合知识·新兴架构域.md — 微服务/云原生/大数据/区块链/AIGC 集成(新大纲增量)(commit 短哈希待提交时补)
- [x] 案例分析专题.md — 5 题结构(1 必答+4 选 2)/答题模板/高频题型(架构识别/质量权衡/数据库与缓存设计/微服务拆分)/时间分配(commit 短哈希待提交时补)
- [x] 论文写作专题.md — 4 选 1/2500~3000 字/结构模板(背景→架构设计→质量属性→效果反思)/素材积累/近年题目(commit 短哈希待提交时补)
- [x] 真题与刷题进度.md — 历年真题入口 + 按知识域刷题计划 + 错题登记表(公开,附「可复制到个人记录私有化」说明)(commit 短哈希待提交时补)
- [x] 收尾 — 全量跑 gen_rk.py + check_index.py(18 项全过);完成项移入「已完成」待提交时补 commit 短哈希
